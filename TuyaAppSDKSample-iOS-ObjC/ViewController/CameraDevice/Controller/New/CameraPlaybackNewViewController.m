...
@interface CameraPlaybackNewViewController ()<ThingSmartCameraDelegate, CameraCalendarViewDelegate, CameraCalendarViewDataSource, ThingTimelineViewDelegate, UITableViewDelegate, UITableViewDataSource>
...
- (void)enableMute:(BOOL)isMute {
    [self.cameraDevice enableMute:isMute forPlayMode:ThingSmartCameraPlayModePlayback];
}
...
- (void)deletePlayBackDataWithPlaybackDate:(ThingSmartPlaybackDate *)playbackDate {
...
- (NSDictionary *)playBackSpeedMap {
    if (!_playBackSpeedMap) {
        _playBackSpeedMap = @{@(ThingSmartCameraPlayBackSpeed_05TIMES) : @"0.5x",
