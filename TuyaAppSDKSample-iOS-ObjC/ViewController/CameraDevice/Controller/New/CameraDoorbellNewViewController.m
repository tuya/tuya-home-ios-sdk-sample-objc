...
@interface CameraDoorbellNewViewController ()<TuyaSmartCameraDelegate, TuyaSmartCameraDPObserver>
...
- (void)cameraDidBeginPreview:(id<TuyaSmartCameraType>)camera {
    ...
    [self.cameraDevice enableMute:NO forPlayMode:TuyaSmartCameraPlayModePreview];
    [self talkAction];
}
...
- (void)camera:(id<TuyaSmartCameraType>)camera didOccurredErrorAtStep:(TYCameraErrorCode)errStepCode specificErrorCode:(NSInteger)errorCode {
    if (errStepCode == TY_ERROR_CONNECT_FAILED || errStepCode == TY_ERROR_CONNECT_DISCONNECT) {
        [self stopLoading];
        self.retryButton.hidden = NO;
    } else if (errStepCode == TY_ERROR_START_PLAYBACK_FAILED || errStepCode == TY_ERROR_QUERY_RECORD_DAY_FAILED || errStepCode == TY_ERROR_QUERY_EVENTLIST_SIFT_FAILED || errStepCode == TY_ERROR_QUERY_TIMESLICE_FAILED) {
        [self stopLoading];
        self.retryButton.hidden = NO;
