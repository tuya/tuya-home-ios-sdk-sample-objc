...
@interface CameraCloudViewController ()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource,ThingTimelineViewDelegate,ThingSmartCloudManagerDelegate>
...
- (void)soundAction {
    ...
    [self.cloudManager enableMute:!isMuted success:^{
        ...
        [self failTipWithMessage:NSLocalizedStringFromTable(@"enable mute failed", @"IPCLocalizable", @"")];
    }];
}
        ...
        if (result) {
            if (self.isRecording) {
                ...
                if ([self.cloudManager stopRecord] != 0) {
                    [self failTipWithMessage:NSLocalizedStringFromTable(@"record failed", @"IPCLocalizable", @"")];
                }else {
                    [self successTipWithMessage:NSLocalizedStringFromTable(@"ipc_multi_view_video_saved", @"IPCLocalizable", @"")];
                }
            }else {
...
- (void)pauseAction {
    if (self.isPlaying && self.isPaused) {
        if ([self.cloudManager resumePlayCloudVideo]) {
            [self failTipWithMessage:NSLocalizedStringFromTable(@"fail", @"IPCLocalizable", @"")];
        }else {
    ...
    [self checkPhotoPermision:^(BOOL result) {
        if (result) {
            if ([self.cloudManager snapShoot]) {
                [self successTipWithMessage:NSLocalizedStringFromTable(@"ipc_multi_view_photo_saved", @"IPCLocalizable", @"")];
            }else {
                [self failTipWithMessage:NSLocalizedStringFromTable(@"fail", @"IPCLocalizable", @"")];
            }
        }
...
- (void)checkCloudState:(ThingSmartCloudState)state {
    ...
    switch (state) {
        case ThingSmartCloudStateNoService:
            ...
            [self tipWithMessage:NSLocalizedStringFromTable(@"ipc_cloudstorage_status_off", @"IPCLocalizable", @"")];
            break;
        case ThingSmartCloudStateNoData:
        case ThingSmartCloudStateExpiredNoData:
            [self enableFeatureButtons:NO];
            [self tipWithMessage:NSLocalizedStringFromTable(@"ipc_cloudstorage_noDataTips", @"IPCLocalizable", @"")];
            break;
        case ThingSmartCloudStateLoadFailed:
            [self enableFeatureButtons:NO];
            [self failTipWithMessage:NSLocalizedStringFromTable(@"ty_network_error", @"IPCLocalizable", @"")];
            break;
...
- (void)loadTimePieceForDay:(ThingSmartCloudDayModel *)dayModel {
    ...
    [self.cloudManager timeLineWithCloudDay:dayModel success:^(NSArray<ThingSmartCloudTimePieceModel *> *timePieces) {
        ...
        [self failTipWithMessage:NSLocalizedStringFromTable(@"ipc_errormsg_data_load_failed", @"IPCLocalizable", @"")];
    }];
...
- (void)gotoCloudServicePanel {
    [self alertWithMessage:NSLocalizedStringFromTable(@"please import ThingSmartCloudServiceBizBundle", @"IPCLocalizable", @"")];
...
- (void)tipWithMessage:(NSString *)text {
    [SVProgressHUD showInfoWithStatus:text];
}

- (void)successTipWithMessage:(NSString *)text {
    [SVProgressHUD showSuccessWithStatus:text];
}

- (void)failTipWithMessage:(NSString *)text {
...
- (void)playCloudTimePiece:(ThingSmartCloudTimePieceModel *)pieceModel playTime:(NSInteger)playTime {
    ...
    [self.cloudManager playCloudVideoWithStartTime:playTime endTime:self.selectedDay.endTime isEvent:NO onResponse:^(int errCode) {
        if (errCode) {
            [weakSelf failTipWithMessage:NSLocalizedStringFromTable(@"ipc_status_stream_failed", @"IPCLocalizable", @"")];
        }else {
            self.isPlaying = YES;
            self.isPaused = NO;
        }
    } onFinished:^(int errCode) {
        [weakSelf tipWithMessage:NSLocalizedStringFromTable(@"ipc_video_end", @"IPCLocalizable", @"")];
    }];
}
