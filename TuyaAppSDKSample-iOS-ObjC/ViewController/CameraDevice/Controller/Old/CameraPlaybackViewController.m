...
@interface CameraPlaybackViewController ()<ThingSmartCameraDelegate, CameraCalendarViewDelegate, CameraCalendarViewDataSource, ThingTimelineViewDelegate>
...
@property (nonatomic, strong) ThingCameraTimeLabel *timeLineLabel;

@property (nonatomic, strong) CameraVideoView *videoView;

@property (nonatomic, strong) NSMutableArray<NSNumber *> *playbackDays;

@property (nonatomic, strong) ThingSmartPlaybackDate *currentDate;
...
@property (nonatomic, strong) id<ThingSmartCameraType> cameraType;
...
@property (nonatomic, copy) ThingSuccessList getPlaybackDaysComplete;

@property (nonatomic, copy) ThingSuccessList getRecordTimeSlicesComplete;
...
- (instancetype)initWithDeviceId:(NSString *)devId {
    if (self = [super initWithNibName:nil bundle:nil]) {
        _needsReconnect = YES;
        _devId = devId;
        _device = [ThingSmartDevice deviceWithDeviceId:devId];
        _cameraType = [ThingSmartCameraFactory cameraWithP2PType:@(_device.deviceModel.p2pType) deviceId:_device.deviceModel.devId delegate:self];
        _muted = YES;
...
- (void)dateAction {
    ...
    ThingSmartPlaybackDate *playbackDate = [ThingSmartPlaybackDate new];
...
- (void)enableMute:(BOOL)isMute {
    [self.cameraType enableMute:isMute forPlayMode:ThingSmartCameraPlayModePlayback];
}
...
- (void)timelineViewDidScroll:(ThingTimelineView *)timeLineView time:(NSTimeInterval)timeInterval isDragging:(BOOL)isDragging {
    ...
    self.timeLineLabel.timeStr = [NSDate thingsdk_timeStringWithTimeInterval:timeInterval timeZone:[NSTimeZone localTimeZone]];
}
...
- (void)calendarView:(CameraCalendarView *)calendarView didSelectYear:(NSInteger)year month:(NSInteger)month day:(NSInteger)day date:(NSDate *)date {
    [calendarView hide];
    [self getRecordAndPlay:[ThingSmartPlaybackDate playbackDateWithDate:date]];
}

#pragma mark - Private

- (void)getRecordAndPlay:(ThingSmartPlaybackDate *)playbackDate {
...
- (void)requestTimeSliceWithPlaybackDate:(ThingSmartPlaybackDate *)date complete:(void(^)(TYDictArray *result))complete {
    ...
    if (![days containsObject:@(date.day)] && ![ThingSmartPlaybackDate isToday:date]) {
        !complete?:complete(@[]);
    }else {
...
- (void)cameraDidConnected:(id<ThingSmartCameraType>)camera {
    ...
    NSDictionary *config = [ThingSmartP2pConfigService getCachedConfigWithDeviceModel:self.device.deviceModel];
...
- (void)camera:(id<ThingSmartCameraType>)camera didReceiveMuteState:(BOOL)isMute playMode:(ThingSmartCameraPlayMode)playMode {
    if (playMode == ThingSmartCameraPlayModePlayback) {
        self.muted = isMute;
    }
...
- (ThingTimelineView *)timeLineView {
    if (!_timeLineView) {
        _timeLineView = [[ThingTimelineView alloc] initWithFrame:CGRectMake(0, self.videoContainer.bottom, [UIScreen mainScreen].bounds.size.width, 150)];
...
- (ThingCameraTimeLabel *)timeLineLabel {
    if (!_timeLineLabel) {
        _timeLineLabel = [[ThingCameraTimeLabel alloc] initWithFrame:CGRectMake((self.timeLineView.width - 74) / 2, self.timeLineView.top, 74, 22)];
        _timeLineLabel.position = 2;
        _timeLineLabel.hidden = YES;
        _timeLineLabel.thing_backgroundColor = [UIColor blackColor];
