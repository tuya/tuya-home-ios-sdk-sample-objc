...
@interface CameraDevice ()<TuyaSmartCameraDelegate>

@property (nonatomic, strong) id<TuyaSmartCameraType> camera;

@property (nonatomic, strong, readwrite) TuyaSmartCameraDPManager *dpManager;

@property (nonatomic, strong) NSHashTable<id<TuyaSmartCameraDelegate>> *innerDelegates;
...
- (instancetype)initWithDeviceId:(NSString *)devId {
    ...
    if (self) {
        _camera = [TuyaSmartCameraFactory cameraWithP2PType:@(self.deviceModel.p2pType) deviceId:self.deviceModel.devId delegate:self];
        _cameraModel = [[CameraDeviceModel alloc] init];
        _innerDelegates = [NSHashTable weakObjectsHashTable];
        
        TuyaSmartCameraAbility *cameraAbility = [TuyaSmartCameraAbility cameraAbilityWithDeviceModel:self.deviceModel];
...
- (TuyaSmartCameraDPManager *)dpManager {
    if (!_dpManager) {
        _dpManager = [[TuyaSmartCameraDPManager alloc] initWithDeviceId:self.deviceModel.devId];
    }
    return _dpManager;
}
...
- (void)connect {
    [self connectWithPlayMode:TuyaSmartCameraPlayModePlayback];
}
...
- (void)cameraDidConnected:(id<TuyaSmartCameraType>)camera {
    ...
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)camera:(id<TuyaSmartCameraType>)camera definitionChanged:(TuyaSmartCameraDefinition)definition {
    self.cameraModel.HD = definition >= TuyaSmartCameraDefinitionHigh;
 ...
 */
- (void)camera:(id<TuyaSmartCameraType>)camera didReceiveEventListSiftData:(NSArray<NSString *> *)titles eventIds:(NSArray<NSNumber *> *)eventIds {
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)camera:(id<TuyaSmartCameraType>)camera didOccurredErrorAtStep:(TYCameraErrorCode)errStepCode specificErrorCode:(NSInteger)errorCode {
    ...
    }else if (errStepCode == TY_ERROR_QUERY_TIMESLICE_FAILED) {
        self.cameraModel.playbackState = CameraDevicePlaybackFailed;
    }else if (errStepCode == TY_ERROR_QUERY_RECORD_DAY_FAILED) {
        self.cameraModel.playbackState = CameraDevicePlaybackFailed;
    } else if (errStepCode == TY_ERROR_QUERY_EVENTLIST_SIFT_FAILED) {
        self.cameraModel.playbackState = CameraDevicePlaybackFailed;
    }
//    } else if (errStepCode == TY_ERROR_SET_PLAYBACK_SPEED_FAILED) {
//
//    }
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)camera:(id<TuyaSmartCameraType>)camera ty_didReceiveFrameData:(const char *)frameData dataSize:(unsigned int)size frameInfo:(TuyaSmartVideoStreamInfo)frameInfo {
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj respondsToSelector:@selector(camera:ty_didReceiveFrameData:dataSize:frameInfo:)]) {
            [obj camera:camera ty_didReceiveFrameData:frameData dataSize:size frameInfo:frameInfo];
        }
    }];
}
...
- (void)camera:(id<TuyaSmartCameraType>)camera ty_didReceiveVideoFrame:(CMSampleBufferRef)sampleBuffer frameInfo:(TuyaSmartVideoFrameInfo)frameInfo {
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj respondsToSelector:@selector(camera:ty_didReceiveVideoFrame:frameInfo:)]) {
            [obj camera:camera ty_didReceiveVideoFrame:sampleBuffer frameInfo:frameInfo];
        }
    }];
}
...
- (void)camera:(id<TuyaSmartCameraType>)camera ty_didRecieveAudioRecordDataWithPCM:(const unsigned char*)pcm length:(int)length sampleRate:(int)sampleRate {
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj respondsToSelector:@selector(camera:ty_didRecieveAudioRecordDataWithPCM:length:sampleRate:)]) {
            [obj camera:camera ty_didRecieveAudioRecordDataWithPCM:pcm length:length sampleRate:sampleRate];
        }
    }];
}
...
- (void)camera:(id<TuyaSmartCameraType>)camera ty_didSpeedPlayWithSpeed:(TuyaSmartCameraPlayBackSpeed)playBackSpeed {
    NSArray<id<TuyaSmartCameraDelegate>> *delegates = self.innerDelegates.allObjects;
    [delegates enumerateObjectsUsingBlock:^(id<TuyaSmartCameraDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj respondsToSelector:@selector(camera:ty_didSpeedPlayWithSpeed:)]) {
            [obj camera:camera ty_didSpeedPlayWithSpeed:playBackSpeed];
        }
    }];
}
...
- (void)enableMute:(BOOL)muted {
    ...
    TuyaSmartCameraPlayMode playMode = isOnPreviewMode ? TuyaSmartCameraPlayModePreview : TuyaSmartCameraPlayModePlayback;
