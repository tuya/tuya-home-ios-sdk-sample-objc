...
@interface CameraCollectionPointListView ()<ThingSmartPTZManagerDeletate, UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSArray<ThingCameraCollectionPointModel *> *dataSource;
...
#pragma mark - action
- (void)renameAction:(ThingCameraCollectionPointModel *)model {
...
- (void)removeAction:(ThingCameraCollectionPointModel *)model {
    if (![self.ptzManager couldOperateCollectionPoint]) {
        [SVProgressHUD showInfoWithStatus:NSLocalizedStringFromTable(@"Not editable in site patrol mode", @"IPCLocalizable", @"")];
        return;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ThingCameraCollectionPointModel *model = self.dataSource[indexPath.row];
    cell.model = model;
    __weak typeof(self) weakSelf = self;
    cell.didClickRenameButton = ^(ThingCameraCollectionPointModel * _Nonnull model) {
        [weakSelf renameAction:model];
    };
    cell.didClickRemoveButton = ^(ThingCameraCollectionPointModel * _Nonnull model) {
