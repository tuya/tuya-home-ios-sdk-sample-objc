...
- (void)reloadData:(NSDictionary *)data{
    NSArray *array = nil;
    if (self.isZigbee){
        array = data[@"scheduleList"];
    }
    else{
        array = data[@"scheduleDetails"];
    }
    ...
    if (self.isZigbee){
        self.startTimeView.contentTextField.text = [NSString stringWithFormat:@"%d",scheduleEffectiveTime/100];
        self.endTimeView.contentTextField.text = [NSString stringWithFormat:@"%d",scheduleInvalidTime/100];
    }
    [self.weekSelectView reloadData:bin_workingDay isZigbee:self.isZigbee];
}
...
- (ThingSmartBLELockScheduleList *)getScheduleListModel{
    ...
    //16进制转成10进制
    NSInteger workingDay = strtoul([self.weekSelectView getScheduleModelWorkingDay:self.isZigbee].UTF8String, 0, 16);
    ...
    if (self.isZigbee){
        scheduleModel.effectiveTime = 100 * startTime;
        scheduleModel.invalidTime = 100 * endTime;
    }
...
- (void)setupViews{
    ...
    self.pwdValueInputView.contentTextField.keyboardType = UIKeyboardTypeNumberPad;
...
- (NSInteger )getEffectiveTime{
    return [[self getEffectiveDate] timeIntervalSince1970];
}

- (NSInteger )getInvalidTime{
    return [[self getInvalidDate] timeIntervalSince1970];
}
