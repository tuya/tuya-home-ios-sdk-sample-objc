...
- (void)setupViews{
    ...
    self.permissionTitle = [[UILabel alloc] initWithFrame:CGRectZero];
    self.permissionTitle.textColor = [UIColor blueColor];
    self.permissionTitle.font = [UIFont systemFontOfSize:16];
    self.permissionTitle.textAlignment = NSTextAlignmentLeft;
    self.permissionTitle.text = @"权限设置";
    self.permissionTitle.hidden = YES;
    self.permissionTitle.userInteractionEnabled = YES;
    
    self.permissionValue = [[UILabel alloc] initWithFrame:CGRectZero];
    self.permissionValue.textColor = [UIColor blueColor];
    self.permissionValue.font = [UIFont systemFontOfSize:16];
    self.permissionValue.textAlignment = NSTextAlignmentRight;
    self.permissionValue.text = @">";
    self.permissionValue.hidden = YES;
    self.permissionValue.userInteractionEnabled = YES;
    ...
    [self addSubview:self.permissionTitle];
    [self addSubview:self.permissionValue];
    ...
    [self.permissionTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self).with.offset(30);
        make.top.equalTo(self.voiceLabel.mas_bottom).with.offset(20);
        make.width.mas_equalTo(100);
        make.height.mas_equalTo(40);
    }];
    
    [self.permissionValue mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self).with.offset(-30);
        make.top.equalTo(self.voiceLabel.mas_bottom).with.offset(20);
        make.width.mas_equalTo(100);
        make.height.mas_equalTo(40);
    ...
    [self.permissionValue bk_whenTapped:^{
        if (self.delegate && [self.delegate respondsToSelector:@selector(permissionAction)]){
            [self.delegate permissionAction];
        }
    }];
}
...
- (void)setupType:(BOOL)type{
    if (self.isZigbee){
        self.permissionTitle.hidden = !type;
        self.permissionValue.hidden = !type;
    }
}
